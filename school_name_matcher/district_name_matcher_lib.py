...
def read_state_districts_csv(path: Text) -> pd.DataFrame:
...
def best_effort_merge_nces_data(
    state_df: pd.DataFrame, nces_districts_df: pd.DataFrame
) -> pd.DataFrame:
  state_abbrev = list(set(state_df["StateAbbrev"]))[0]
  # Get NCES data for state.
  state_schools_df = nces_districts_df[
    nces_districts_df["state"] == state_abbrev]
  state_schools_df.reset_index(drop=True, inplace=True)
  districts_search_index =\
  ...
  for index, row in state_df.iterrows():
    ...
    scored_results = trigger_school_name_results(
        query, state_schools_df, districts_search_index)
    has_results = False
